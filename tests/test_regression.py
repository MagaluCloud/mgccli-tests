...
def test_invalid_param_hang_fix_1_0():
    exit_code, stdout, stderr, _ = run_cli(["vm", "instances", "create", "-a", "whatever"], timeout=2)
    assert exit_code == 1, stderr
    assert "unknown shorthand flag" in stderr

def test_invalid_param_hang_fix_1_1():
    exit_code, stdout, stderr, _ = run_cli(["vm", "instances", "create", "-awhatever"], timeout=2)
    assert exit_code == 1, stderr
    assert "unknown shorthand flag" in stderr


def test_invalid_param_hang_fix_2_0():
    _, _, stderr, _ = run_cli(["--debug=teste"], timeout=2)
